...
def _reshape_norm(data, channel_axis=None, normalize_params={"normalize": False}):
    ...
    """
    if np.array([td.ndim != 3 for td in data]).sum() > 0 or np.array([td.shape[0] != 3 for td in data]).sum() > 0:
        data_new = []
        for td in data:
            if td.ndim == 3:
...
def _get_batch(inds, data=None, labels=None, files=None, labels_files=None, normalize_params={"normalize": False}):
    """
...
def _reshape_norm_save(files, channels=None, channel_axis=None, normalize_params={"normalize": False}):
...
def _process_train_test(
    ...
    if (test_labels is not None and nimg_test != len(test_labels)) or (test_labels_files is not None and nimg_test != len(test_labels_files)):
    ...
    if train_labels is not None:
        ...
        for k in trange(nimg):
            tl = dynamics.labels_to_flows(io.imread(train_labels_files), files=train_files, device=device)
        if test_files is not None:
            for k in trange(nimg_test):
                tl = dynamics.labels_to_flows(io.imread(test_labels_files), files=test_files, device=device)
    ...
    for k in trange(nimg):
        tl = train_labels[k][0] if train_labels is not None else io.imread(train_labels_files[k])[0]
        diam_train[k], dall = utils.diameters(tl)
        nmasks[k] = len(dall)
    diam_train[diam_train < 5] = 5.0
    if test_data is not None:
        diam_test = np.array([utils.diameters(test_labels[k][0])[0] for k in trange(len(test_labels))])
    ...
    train_probs = 1.0 / nimg * np.ones(nimg, "float64") if train_probs is None else train_probs
    train_probs /= train_probs.sum()
    if test_files is not None or test_data is not None:
        test_probs = 1.0 / nimg_test * np.ones(nimg_test, "float64") if test_probs is None else test_probs
...
def train_seg(
    ...
    for iepoch in range(n_epochs):
        ...
        for k in range(0, nimg_per_epoch, batch_size):
            ...
            inds = rperm[k:kend]
            imgs, lbls = _get_batch(inds, data=train_data, labels=train_labels, files=train_files, labels_files=train_labels_files, **kwargs)
            ...
            if test_data is not None or test_files is not None:
                ...
                for ibatch in range(0, len(rperm), batch_size):
                    with torch.no_grad():
                        ...
                        imgs, lbls = _get_batch(inds, data=test_data, labels=test_labels, files=test_files, labels_files=test_labels_files, **kwargs)
