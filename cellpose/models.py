...
class CellposeModel:
    ...
    def __init__(self, gpu=False, pretrained_model="cpsam", model_type=None, diam_mean=None, device=None, nchan=None, use_bfloat16=True):
        """
        Initialize the CellposeModel.
    ...
    def eval(
        ...
        if nimg > 1 and do_normalization and (stitch_threshold or do_3D):
            ...
            if normalize_params["norm3D"] and nimg > 1 and do_normalization:
                models_logger.warning("normalize_params['norm3D'] is True but do_3D is False and stitch_threshold=0, so setting to False")
        ...
        dP, cellprob, styles = self._run_net(
            x, augment=augment, batch_size=batch_size, tile_overlap=tile_overlap, bsize=bsize, do_3D=do_3D, anisotropy=anisotropy
        )
    ...
    def _run_net(self, x, augment=False, batch_size=8, tile_overlap=0.1, bsize=224, anisotropy=1.0, do_3D=False):
        ...
        if do_3D:
            ...
            if anisotropy is not None and anisotropy != 1.0:
                ...
                x = transforms.resize_image(x.transpose(1, 0, 2, 3), Ly=int(Lz * anisotropy), Lx=int(Lx)).transpose(1, 0, 2, 3)
            yf, styles = run_3D(self.net, x, batch_size=batch_size, augment=augment, tile_overlap=tile_overlap, bsize=bsize)
            cellprob = yf[..., -1]
    ...
    def _compute_masks(
        self, shape, dP, cellprob, flow_threshold=0.4, cellprob_threshold=0.0, min_size=15, max_size_fraction=0.4, niter=None, do_3D=False, stitch_threshold=0.0
    ):
